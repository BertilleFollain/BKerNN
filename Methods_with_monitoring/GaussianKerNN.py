...
class GaussianKerNN(RegressorMixin, BaseEstimator):
    """
    This method implements a regressor based on a fusions between kernel ridge regression with the gaussian kernel and
    ...
    def fit(self, X, y, gamma=1, max_iter=100, beta_0=1.5, beta_1=0.5, random_state=0, backtracking=True,
            monitoring=True, X_test=None, y_test=None, k=None, p=None):
        """
        ...
        if monitoring:
            if y_test is None:
                y_test = np.hstack([y, 0])
            if X_test is None:
                X_test = np.vstack([X, np.zeros(self.n_features_in_)])
            self.X_test_ = X_test
            self.y_test_ = y_test
            if k is None:
            ...
            self.k_ = k
            self.p_ = p
            self.training_error_ = np.zeros(self.max_iter_)
            self.val_ = np.zeros(self.max_iter_)
            self.testing_error_ = np.zeros(self.max_iter_)
            self.feature_learning_score_ = np.zeros(self.max_iter_)
            self.dimension_score_ = np.zeros(self.max_iter_)
            self.training_score_ = np.zeros(self.max_iter_)
            self.testing_score_ = np.zeros(self.max_iter_)
        ...
        for iteration in range(self.max_iter_):
            ...
            if monitoring:
                y_pred = self.K_ @ self.alpha_ + self.y_mean_ - np.mean(self.K_ @ self.alpha_)
                self.training_error_[iteration] = np.mean((y_pred - y) ** 2)  # stores current mean squared error
                self.val_[iteration] = (self.lambda_val / 2) * np.dot(self.y_norm_.T, self.alpha_) + self.penalty()
                K_test = self.K_from_W(self.X_, X_test, self.W_, self.m)
                y_test_pred = K_test @ self.alpha_ + self.y_mean_ - np.mean(self.K_ @ self.alpha_)
                self.testing_error_[iteration] = np.mean((y_test_pred - y_test) ** 2)
                self.training_error_[iteration] = np.mean((y_pred - y) ** 2)  # stores current mean squared error
                self.training_score_[iteration] = self.score(self.X_, y)
                self.testing_score_[iteration] = self.score(X_test, y_test)
                self.dimension_score_[iteration] = self.dimension_score(k)
                self.feature_learning_score_[iteration] = self.feature_learning_score(p)
    ...
    def compute_gradient(alpha, m, n_features_in, n, W, X, lambda_val):
        ...
        for j in range(m):
            ...
            for a in prange(n):
                for b in prange(n):
                    sign_dot[a, b] = (product[a] - product[b])[0] * np.exp(-np.abs(product[a] - product[b]) ** 2 / 2)[0]
            for l in range(n_features_in):
                for c in prange(n):
                    for d in prange(n):
                        diff[c, d] = (X[c, l] - X[d, l])
                gradW[j * n_features_in + l] = lambda_val * np.sum(Z * sign_dot * diff) / (2 * m)
        return gradW
    ...
    def K_from_W(own_X, X, W, m):
        ...
        for i in prange(n):
            for i_prime in prange(n_test):
                ...
                exp_dot_product = np.exp(-np.abs(dot_product) ** 2 / 2)
                K[i_prime, i] = np.sum(exp_dot_product) / m
        return K
