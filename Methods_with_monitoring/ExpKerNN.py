...
class ExpKerNN(RegressorMixin, BaseEstimator):
    """
    This method implements a regressor based on a fusions between kernel ridge regression with the exponential kernel and
    ...
    def compute_gradient(alpha, m, n_features_in, n, W, X, lambda_val):
        ...
        for j in range(m):
            ...
            for a in prange(n):
                for b in prange(n):
                    sign_dot[a, b] = np.sign(product[a] - product[b])[0] * np.exp(-np.abs(product[a] - product[b]) / 2)[
                        0]
    ...
    def K_from_W(own_X, X, W, m):
        ...
        for i in prange(n):
            for i_prime in prange(n_test):
                ...
                exp_dot_product = np.exp(-np.abs(dot_product) / 2)
